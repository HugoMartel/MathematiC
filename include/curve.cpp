/*
    sdlgui/curve.cpp -- Simple curve widget for showing a function plot
...
struct Curve::AsyncTexture
{
  ...
  void load(Curve* ptr)
  {
    ...
    std::thread tgr([=]() {
      ...
      nvgFillColor(ctx, curve->backgroundColor().toNvgColor());
      nvgFill(ctx);
      ...
      nvgStroke(ctx);
      nvgFillColor(ctx, curve->foregroundColor().toNvgColor());
      nvgFill(ctx);
...
Curve::Curve(Widget *parent, const std::string &caption)
...
Vector2i Curve::preferredSize(SDL_Renderer *) const
{
