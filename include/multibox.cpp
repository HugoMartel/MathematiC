/*
    sdlgui/multibox.cpp -- Fancy text box with builtin regular
    expression-based validation and multiline support
...
#include "../lib/sdlgui/nanovg.h"
#define NANOVG_RT_IMPLEMENTATION
#define NANORT_IMPLEMENTATION
#include "../lib/sdlgui/nanovg_rt.h"

NAMESPACE_BEGIN(sdlgui)

struct MultiBox::AsyncTexture
{
  int id;
  Texture tex;
  ...
  void load(MultiBox* ptr, bool editable, bool focused, bool validFormat, bool outside)
  {
    MultiBox* tbox = ptr;
...
MultiBox::MultiBox(Widget *parent,const std::string &value, const std::string& units)
    : Widget(parent),
      mEditable(false),
      mSpinnable(false),
      mCommitted(true),
...
void MultiBox::setEditable(bool editable) 
{
    mEditable = editable;
    _captionTex.dirty = true;
...
void MultiBox::setTheme(Theme *theme) 
{
...
Vector2i MultiBox::preferredSize(SDL_Renderer *ctx) const
{
  Vector2i size(0, fontSize() * 1.4f);
    ...
    {
        uw = const_cast<MultiBox*>(this)->mTheme->getUtf8Width("sans", fontSize(), mUnits.c_str());
    }
    float sw = 0;
    if (mSpinnable) 
    {
    ...
    float ts = const_cast<MultiBox*>(this)->mTheme->getUtf8Width("sans", fontSize(), mValue.c_str());
    size.x = size.y + ts + uw + sw;
    return size;
}

void MultiBox::drawBody(SDL_Renderer* renderer)
{
...
void MultiBox::draw(SDL_Renderer* renderer) 
{
    ...
    else if (!mUnits.empty()) 
    {
      if (_unitsTex.dirty)
      /* TODO:  
      * add a way to split text every \n, and render it into different boxes
      * rendered here, i guess so could be modified here
      */
...
bool MultiBox::mouseButtonEvent(const Vector2i &p, int button, bool down,
                               int modifiers)
{
...
bool MultiBox::mouseMotionEvent(const Vector2i &p, const Vector2i & /* rel */,
...
bool MultiBox::mouseDragEvent(const Vector2i &p, const Vector2i &/* rel */,
                             int /* button */, int /* modifiers */) 
{
    mMousePos = p;
    mMouseDragPos = p;
...
bool MultiBox::focusEvent(bool focused) 
{
...
/*
* TT added return key detection
*/
bool MultiBox::keyboardEvent(int key, int /* scancode */, int action, int modifiers) {
    if (mEditable && focused()) {
        if (action == SDL_PRESSED) {
            ...
            /* EDIT HERE to add new line, checking enter, enter numpad and idk which one
            * check the doc on https://wiki.libsdl.org/SDL_Keycode
            */
            } else if (key == SDLK_RETURN2 || key == SDLK_RETURN || key == SDLK_KP_ENTER) {
                /* add code to return to line, create new MultiBox with same buffer ?
                * the buffer is mValueTemp, then parsed into 
                */
                /* begin of added code */
                mValueTemp.push_back('\n');
                if (!mCommitted)
                    focusEvent(false);
...
bool MultiBox::keyboardCharacterEvent(unsigned int codepoint) 
{
    if (mEditable && focused()) 
    {
...
/*TODO: check here if it can accept \n; and if so if it's not destroying the input*/
bool MultiBox::checkFormat(const std::string &input, const std::string &format) 
{
    if (format.empty())
        return true;
    try 
...
void MultiBox::pasteFromClipboard() 
{
    Screen *sc = dynamic_cast<Screen *>(this->window()->parent());
...
void MultiBox::updateCursor(float lastx, const std::string& str) 
{
...
float MultiBox::cursorIndex2Position(int index, float lastx, const std::string& str) 
{
    float pos = 0;
    if (index >= str.size())
...
int MultiBox::position2CursorIndex(float posx, float lastx, const std::string& str) 
{
    int mCursorId = 0;
...
MultiBox::SpinArea MultiBox::spinArea(const Vector2i & pos)
{
