/*
    sdl_gui/multibox.h -- Fancy text box with builtin regular
...
/**
 * \class MultiBox multibox.h sdl_gui/multibox.h
 *
 * \brief Fancy text box with builtin regular expression-based validation.
 */
class  MultiBox : public Widget 
{
public:
    ...
    MultiBox(Widget *parent, const std::string &value = "Untitled", const std::string& units = "");
    ...
    MultiBox& withAlignment(Alignment align) { setAlignment(align); return *this; }
...
/**
 * \class IntBox multibox.h sdl_gui/multibox.h
 *
 * \brief A specialization of MultiBox for representing integral values.
 *
 * Template parameters should be integral types, e.g. ``int``, ``long``,
 * ``uint32_t``, etc.
 */
template <typename Scalar>
class IntBox : public MultiBox {
public:
    IntBox(Widget *parent, Scalar value = (Scalar) 0) : MultiBox(parent) {
        setDefaultValue("0");
    ...
    Scalar value() const {
        std::istringstream iss(MultiBox::value());
        Scalar value = 0;
        iss >> value;
        return value;
    }
    ...
    void setValue(Scalar value) {
        ...
        MultiBox::setValue(std::to_string(clampedValue));
    }

    void setCallback(const std::function<void(Scalar)> &cb) {
        MultiBox::setCallback(
    ...
    virtual bool mouseButtonEvent(const Vector2i &p, int button, bool down, int modifiers) override {
        ...
        return MultiBox::mouseButtonEvent(p, button, down, modifiers);
    }
    virtual bool mouseDragEvent(const Vector2i &p, const Vector2i &rel, int button, int modifiers) override {
        if (MultiBox::mouseDragEvent(p, rel, button, modifiers)) {
            return true;
        }
...
/**
 * \class FloatBox multibox.h sdl_gui/multibox.h
 *
 * \brief A specialization of MultiBox representing floating point values.

 * Template parameters should be float types, e.g. ``float``, ``double``,
 * ``float64_t``, etc.
 */
template <typename Scalar>
class FloatBox : public MultiBox 
{
public:
    FloatBox(Widget *parent, Scalar value = (Scalar) 0.f) : MultiBox(parent) {
    ...
    Scalar value() const {
        return (Scalar) std::stod(MultiBox::value());
    }
    ...
    void setValue(Scalar value) {
        ...
        MultiBox::setValue(buffer);
    }

    void setCallback(const std::function<void(Scalar)> &cb) {
        MultiBox::setCallback([cb, this](const std::string &str) {
